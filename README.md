...
개인 맞춤형 웹 대시보드 서비스로, 하나의 화면에서 일상에 필요한 다양한 정보를 위젯 단위로 확인하고 관리할 수 있는 SPA(Single Page Application)이다. 위젯은 금융·정보·생산성 세 개의 섹션으로 분류하여 탐색한다.
...
- **레이아웃:** 위젯 섹션을 좌우 슬라이드로 탐색, 키보드 ←→ 지원
- **위젯 배경 테마:** 위젯별 8색 색상 선택
...
Google Calendar는 API 호출 자체를 프론트엔드(브라우저)에서 수행한다. 사용자가 Google 로그인 팝업을 통해 직접 액세스 토큰을 발급받고, 그 토큰으로 브라우저가 Google Calendar API를 직접 호출하는 구조다. 백엔드는 `GOOGLE_CLIENT_ID`를 `/api/config` 엔드포인트를 통해 프론트에 전달하는 역할만 한다. `GOOGLE_CLIENT_ID`는 OAuth 2.0 규격에서 의도적으로 공개하도록 설계된 값으로, 노출되어도 보안상 문제가 없다. 보안은 Client ID가 아닌 사용자 본인의 로그인을 통한 토큰 발급 과정에서 보장된다.
...
- **저장 방식:** Zustand의 `persist` 미들웨어가 스토어 상태 변경 저장
...
| SPA 폴백       | 존재하지 않는 경로 요청 → `index.html`로 처리 (새로고침·직접 URL 입력 시 404 방지) |
...
외부에서 보이는 포트는 **80번(Nginx) 하나뿐**이며, Express 서버는 내부 포트 4000번에서만 동작한다.
...
[CI] 의존성 설치 → Lint → Build  ← 실패 시 배포 중단
       ↓ 성공
[CD] GitHub Actions이 Render Deploy Hook URL 호출
       ↓
Render가 Docker 이미지 빌드 후 배포
```
...
| 단계        | 명령                                   | 내용                                                                      |
| ----------- | -------------------------------------- | ------------------------------------------------------------------------- |
| **Install** | `npm ci`                               | 프론트엔드 의존성 설치 (package-lock.json)                                |
| **Lint**    | `npm run lint`                         | ESLint로 소스 코드 규칙 위반 검사.                                        |
